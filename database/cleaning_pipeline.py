...
CSV_FILE = "raw/openpowerlifting-2025-05-31-c2a0b8b0.csv"
DB_FILE = "sqlite3_db/powerlifting.db"
SCHEMA_PATH = "sqlite3_db/schema.txt"

if not Path(CSV_FILE).exists():
    raise ValueError(
        "Please download the openpowerlifting-2025-05-31-c2a0b8b0.csv (other dates are acceptable) from https://gitlab.com/openpowerlifting/opl-data"
    )
elif Path(DB_FILE).exists():
    raise ValueError(
        f"Database {DB_FILE} already exists. Please rename the existing database or choose a new database name"
    )
elif not Path(SCHEMA_PATH).exists():
    raise ValueError(
        f"Could not find schema at {SCHEMA_PATH}"
    )
##################################
...
meets = df[meet_cols].drop_duplicates().dropna(subset=['Federation']).reset_index(drop=True)
...
meets['meet_id'] = meets.index + 1 # need to do this manually for later even though sql does it automatically
...
lifter_df = df[lifter_cols].drop_duplicates().dropna(subset=['Name', 'Sex']).reset_index(drop=True)
...
lifters = lifter_df.drop_duplicates().drop_duplicates(subset=['Name']).reset_index(drop=True)
...
result_cols = [
    'lifter_id', 'meet_id', 'Event', 'Equipment', 'Age', 'Division',
    'BodyweightKg', 'WeightClassKg',
    'Squat1Kg', 'Squat2Kg', 'Squat3Kg', 'Squat4Kg', 'Best3SquatKg',
    'Bench1Kg', 'Bench2Kg', 'Bench3Kg', 'Bench4Kg', 'Best3BenchKg',
    'Deadlift1Kg', 'Deadlift2Kg', 'Deadlift3Kg', 'Deadlift4Kg', 'Best3DeadliftKg',
    'TotalKg', 'Place', 'Dots', 'Wilks', 'Tested'
]
results = df[result_cols].drop_duplicates().dropna(subset=['lifter_id', 'meet_id', 'Event', 'Equipment', 'Place']).reset_index(drop=True)
